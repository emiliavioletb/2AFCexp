...
# Initialize components for Routine "intrialBreak"
intrialBreakClock = core.Clock()
intrialBreakText = visual.TextStim(win=win, name='intrialBreakText',
    text='default text',
    font='Arial',
    ...
    languageStyle='LTR',
    depth=0.0);
intrialTimer = visual.TextStim(win=win, name='intrialTimer',
    text='default text',
    font='Arial',
...
# Initialize components for Routine "intrialBreak2"
intrialBreak2Clock = core.Clock()
intrial2BreakText = visual.TextStim(win=win, name='intrial2BreakText',
    ...
    languageStyle='LTR',
    depth=0.0);
intrial2BreakKey_Resp = keyboard.Keyboard()
...
# ------Prepare to start Routine "intrialBreak"-------
continueRoutine = True
routineTimer.add(30.000000)
# update component parameters for each repeat
# keep track of which components have finished
intrialBreakComponents = [intrialBreakText, intrialTimer]
for thisComponent in intrialBreakComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
intrialBreakClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "intrialBreak"-------
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = intrialBreakClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=intrialBreakClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *intrialBreakText* updates
    if intrialBreakText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        intrialBreakText.frameNStart = frameN  # exact frame index
        intrialBreakText.tStart = t  # local t and not account for scr refresh
        intrialBreakText.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(intrialBreakText, 'tStartRefresh')  # time at next scr refresh
        intrialBreakText.setAutoDraw(True)
    if intrialBreakText.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > intrialBreakText.tStartRefresh + 30-frameTolerance:
            # keep track of stop time/frame for later
            intrialBreakText.tStop = t  # not accounting for scr refresh
            intrialBreakText.frameNStop = frameN  # exact frame index
            win.timeOnFlip(intrialBreakText, 'tStopRefresh')  # time at next scr refresh
            intrialBreakText.setAutoDraw(False)
    if intrialBreakText.status == STARTED:  # only update if drawing
        intrialBreakText.setText('Take a moment to relax before continuing.', log=False)
    
    # *intrialTimer* updates
    if intrialTimer.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        intrialTimer.frameNStart = frameN  # exact frame index
        intrialTimer.tStart = t  # local t and not account for scr refresh
        intrialTimer.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(intrialTimer, 'tStartRefresh')  # time at next scr refresh
        intrialTimer.setAutoDraw(True)
    if intrialTimer.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > intrialTimer.tStartRefresh + 30-frameTolerance:
            # keep track of stop time/frame for later
            intrialTimer.tStop = t  # not accounting for scr refresh
            intrialTimer.frameNStop = frameN  # exact frame index
            win.timeOnFlip(intrialTimer, 'tStopRefresh')  # time at next scr refresh
            intrialTimer.setAutoDraw(False)
    if intrialTimer.status == STARTED:  # only update if drawing
        intrialTimer.setText(str(round(routineTimer.getTime()))
, log=False)
...
# -------Ending Routine "intrialBreak"-------
for thisComponent in intrialBreakComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('intrialBreakText.started', intrialBreakText.tStartRefresh)
thisExp.addData('intrialBreakText.stopped', intrialBreakText.tStopRefresh)
thisExp.addData('intrialTimer.started', intrialTimer.tStartRefresh)
thisExp.addData('intrialTimer.stopped', intrialTimer.tStopRefresh)
...
for thisLongBreak in longBreak:
    ...
    # set up handler to look after randomisation of conditions etc
    Section2 = data.TrialHandler(nReps=4, method='random', 
                                 extraInfo=expInfo, originPath=-1,
                                 trialList=[None],
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # ------Prepare to start Routine "intrialBreak2"-------
        continueRoutine = True
        # update component parameters for each repeat
        intrial2BreakKey_Resp.keys = []
        intrial2BreakKey_Resp.rt = []
        _intrial2BreakKey_Resp_allKeys = []
        # keep track of which components have finished
        intrialBreak2Components = [intrial2BreakText, intrial2BreakKey_Resp]
        for thisComponent in intrialBreak2Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        intrialBreak2Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "intrialBreak2"-------
        while continueRoutine:
            # get current time
            t = intrialBreak2Clock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=intrialBreak2Clock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            
            # *intrial2BreakText* updates
            if intrial2BreakText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                intrial2BreakText.frameNStart = frameN  # exact frame index
                intrial2BreakText.tStart = t  # local t and not account for scr refresh
                intrial2BreakText.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(intrial2BreakText, 'tStartRefresh')  # time at next scr refresh
                intrial2BreakText.setAutoDraw(True)
            
            # *intrial2BreakKey_Resp* updates
            waitOnFlip = False
            if intrial2BreakKey_Resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                intrial2BreakKey_Resp.frameNStart = frameN  # exact frame index
                intrial2BreakKey_Resp.tStart = t  # local t and not account for scr refresh
                intrial2BreakKey_Resp.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(intrial2BreakKey_Resp, 'tStartRefresh')  # time at next scr refresh
                intrial2BreakKey_Resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(intrial2BreakKey_Resp.clock.reset)  # t=0 on next screen flip
            if intrial2BreakKey_Resp.status == STARTED and not waitOnFlip:
                theseKeys = intrial2BreakKey_Resp.getKeys(keyList=None, waitRelease=False)
                _intrial2BreakKey_Resp_allKeys.extend(theseKeys)
                if len(_intrial2BreakKey_Resp_allKeys):
                    intrial2BreakKey_Resp.keys = _intrial2BreakKey_Resp_allKeys[-1].name  # just the last key pressed
                    intrial2BreakKey_Resp.rt = _intrial2BreakKey_Resp_allKeys[-1].rt
                    # a response ends the routine
                    continueRoutine = False
        ...
        # -------Ending Routine "intrialBreak2"-------
        for thisComponent in intrialBreak2Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('intrial2BreakText.started', intrial2BreakText.tStartRefresh)
        thisExp.addData('intrial2BreakText.stopped', intrial2BreakText.tStopRefresh)
        # check responses
        if intrial2BreakKey_Resp.keys in ['', [], None]:  # No response was made
            intrial2BreakKey_Resp.keys = None
        thisExp.addData('intrial2BreakKey_Resp.keys',intrial2BreakKey_Resp.keys)
        if intrial2BreakKey_Resp.keys != None:  # we had a response
            thisExp.addData('intrial2BreakKey_Resp.rt', intrial2BreakKey_Resp.rt)
        thisExp.addData('intrial2BreakKey_Resp.started', intrial2BreakKey_Resp.tStartRefresh)
        thisExp.addData('intrial2BreakKey_Resp.stopped', intrial2BreakKey_Resp.tStopRefresh)
        thisExp.nextEntry()
        # the Routine "intrialBreak2" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
    ...
    # completed 4 repeats of 'Section2'
